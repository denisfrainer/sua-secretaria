...
export function generatePrompt(businessName: string, customInstructions: string, services: any[] = [], ownerId: string = "") {
    ...
    const servicesList = services.length > 0 
        ? services.map(s => `- ${s.name}: R$ ${s.price} (${s.duration})`).join('\n')
    ...
    return DEFAULT_SYSTEM_PROMPT
        ...
        .replace("[INJETAR_SERVICOS]", servicesList)
        .replace("[INJETAR_CONTEXTO_DONO]", ownerId ? `O ID do dono deste negócio para ferramentas de agendamento é: ${ownerId}` : "");
}